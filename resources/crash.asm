...
!CRASHDUMP_TILEMAP_BUFFER = $7E5800
!CRASHDUMP = $7EFF00
...
crash_transfer_custom_tileset:
{
    ...
    LDX.w #CrashHudTable : STX $4302 ; Source offset
    LDA.b #CrashHudTable>>16 : STA $4304 ; Source bank
...
CrashViewer:
{
    ...
    JSR crash_transfer_custom_tileset
...
crash_draw4:
{
    ...
    LDA.w CrashHexMenuGFXTable,Y : STA !CRASHDUMP_TILEMAP_BUFFER,X
    ; (0X00)
    LDA !ram_crash_draw_value : AND #$0F00 : XBA : ASL : TAY
    LDA.w CrashHexMenuGFXTable,Y : STA !CRASHDUMP_TILEMAP_BUFFER+2,X
    ; (00X0)
    LDA !ram_crash_draw_value : AND #$00F0 : LSR #3 : TAY
    LDA.w CrashHexMenuGFXTable,Y : STA !CRASHDUMP_TILEMAP_BUFFER+4,X
    ; (000X)
    LDA !ram_crash_draw_value : AND #$000F : ASL : TAY
    LDA.w CrashHexMenuGFXTable,Y : STA !CRASHDUMP_TILEMAP_BUFFER+6,X
    PLP
    RTS
}
...
crash_cgram_transfer:
{
  ...
  .transfer
    ...
    JSL crash_execute_cgram_transfer
    PLP
    PHK : PLB
    RTL
}
...
CrashTextHeader:
    table ../resources/crash_header.tbl
    db "CRASH HANDLER", #$FF
    table ../resources/crash_normal.tbl
